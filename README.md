...
npm run cppx -- init ./myapp --name myapp       # 프로젝트 생성
...
현재는 `schema_version`, `compiler`, `tools`, `presets`, `dependency_backend`가 실제 생성기와 연결됩니다. `[[presets]]`는 configure/build/test/pack 프리셋과 VSCode 생성물에 반영되고, `dependency_backend`는 `vcpkg`, `conan`, `none`을 지원합니다.
