...
- `managed`
  ...
  - 아카이브 기반 관리형 도구는 SHA-256 검증이 끝난 뒤에만 압축을 풉니다.
- `system`
...
`doctor`는 여기서 한 단계 더 나아가 blocker와 warning을 구분해서 보여 줍니다. 특히 macOS/Linux의 `none` backend에서 왜 `cppx add`가 비활성화되는지, `conan` backend에서 무엇이 빠졌는지까지 바로 설명합니다.
...
처음 설정이 막힐 때는 `smoke:native`보다 `npm run cppx -- doctor <workspace>`로 현재 상태를 먼저 확인하는 편이 빠릅니다.
