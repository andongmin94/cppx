...
- [도구 설치](./install.md) — Windows 관리형 설치와 macOS/Linux system 도구 흐름을 설명합니다
- [CLI 사용법](./cli.md) — 프로젝트 초기화부터 빌드·실행·패키징까지 CLI 명령어를 안내합니다
- [설정 (config.toml)](./config.md) — `.cppx/config.toml`로 프로젝트를 세밀하게 제어합니다
- [마이그레이션 가이드](./migration.md) — 레거시 설정, backend, preset 모델 전환 시 확인할 점을 정리합니다
- [GUI 사용법](./gui.md) — Workspace / Build / Logs 탭과 각 카드의 역할을 설명합니다
...
또한 backend, tool policy, preset matrix, CMake 설정도 같은 화면에서 편집할 수 있습니다.
